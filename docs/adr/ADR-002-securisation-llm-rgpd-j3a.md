...
### Négatives
- La séparation system/user (couche 1) dépend de l'API du provider LLM utilisé : certains providers (ex. Ollama via `/api/generate`) ne l'exposent pas nativement, ce qui affaiblit ponctuellement cette couche pour ce provider spécifique — mitigé par les couches 2 à 4, qui restent provider-agnostiques.
- Le format ZIP de l'export RGPD est un choix pragmatique pour cette perturbation ; toute exigence future demandant strictement une réponse JSON directe (sans passer par un fichier) nécessiterait un endpoint complémentaire.
