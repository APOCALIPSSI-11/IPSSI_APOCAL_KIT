...
Critères de choix : domaines variés, tailles variées (court/moyen/long ≤ 5 Mo), ≥ 1 contenu factuel pour tester les hallucinations. Archive dans `docs/pertubations/j2/Etude_benchmarks/corpus/`.
...
**Procédure** : POST API génération → mesure temps wall-clock → vérification JSON 10 QCM × 4 options × 1 correct_index. Stockage : `docs/pertubations/j2/Etude_benchmarks/measurements-latency.csv`.
